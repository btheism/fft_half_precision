...
int main(int argc, char *argv[]) {
    ...
    cudaMallocManaged((void**)&input_half,sizeof(short)*(fft_length+2)*(batch+window_size-step));
    ...
    //全新写入模式
    std::ofstream random_data("random_data_for_1_channel.dat",std::ios::trunc|std::ios::binary);
    ...
    char2float_reflect(input_char+fft_length*(batch-step)-1,input_half+(fft_length+2)*batch,fft_length,window_size-step,thread_num);
    ...
    compress(average_data, input_float, input_float+(fft_length/2+1)*window_size,compressed_data,(fft_length/2+1), ((batch+window_size-step)-(window_size-step))/step, step, begin_channel ,channel_num, window_size, thread_num);
