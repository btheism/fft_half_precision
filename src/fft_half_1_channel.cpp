...
int main(int argc, char *argv[]) {
    ...
    //计算数据被划分的batch数,且要去除无法被step整除的部分
    long long int signal_batch=(signal_length/fft_length/step)*step;
    //缓冲区减去窗口大小是每次压缩的数据量
    ...
    long long int input_char_size=fft_length*(((per_batch+remain_batch)>window_size)?(per_batch+remain_batch):window_size)*(long long int)1;
    long long int input_half_size=(fft_length+2)*batch_buffer_size*(long long int)1;
    ...
    long long int input_half_offset=(fft_length+2)*window_size;
    long long int input_float_offset=(fft_length/2+1)*window_size;
    ...
    readfile(input_char_cpu,file_list,file_size_list,fft_length*window_size);
    printf("copy input char to gpu memory\n");
    cudaMemcpy(input_char,input_char_cpu,fft_length*window_size*sizeof(char),cudaMemcpyHostToDevice);
    //read_char_array(input_char, simulate_input_char ,fft_length*window_size*2);
    char2float(input_char,input_half,fft_length,window_size,thread_num);
    #ifdef PRINT_MEMORY
    print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
    #endif
    
    fft_1d_half(input_half,input_half,fft_length,window_size);
    #ifdef PRINT_MEMORY
    print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
    #endif
    
    complex_modulus_squared(input_half,input_half,fft_length/2,window_size,thread_num);
    #ifdef PRINT_MEMORY
    print_data_float(input_float,0,(fft_length/2+1)*batch_buffer_size,fft_length/2+1);
    #endif

    channels_sum(input_float,average_data,window_size-step,window_size,(fft_length/2+1),fft_length/2,thread_num);
    #ifdef PRINT_MEMORY
    ...
    for(int loop_num=0;loop_num<compress_num;loop_num++)
    {
        ...
        readfile(input_char_cpu,file_list,file_size_list,fft_length*per_batch);
        
        printf("copy input char to gpu memory\n");
        cudaMemcpy(input_char,input_char_cpu,fft_length*per_batch*sizeof(char),cudaMemcpyHostToDevice);
        //read_char_array(input_char, simulate_input_char ,fft_length*per_batch*2);        
        char2float(input_char,input_half+input_half_offset,fft_length,per_batch,thread_num);
        #ifdef PRINT_MEMORY
        print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
        #endif
        
        fft_1d_half(input_half+input_half_offset,input_half+input_half_offset,fft_length,per_batch);
        #ifdef PRINT_MEMORY
        print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
        #endif
        
        complex_modulus_squared(input_half+input_half_offset,input_float+input_float_offset,fft_length/2,per_batch,thread_num);
        #ifdef PRINT_MEMORY
        print_data_float(input_float,0,(fft_length/2+1)*batch_buffer_size,fft_length/2+1);
        #endif
        ...
        cudaMemcpy(input_float,input_float+(fft_length/2+1)*per_batch,(fft_length/2+1)*window_size*sizeof(float),cudaMemcpyDeviceToDevice);
        #ifdef PRINT_MEMORY
        print_data_float(input_float,0,(fft_length/2+1)*batch_buffer_size,fft_length/2+1);
        #endif
    }
     ...
     if(remain_batch>0)
     {
         ...
         readfile(input_char_cpu+fft_length*per_batch,file_list,file_size_list,fft_length*remain_batch);
         
         printf("copy input char to gpu memory\n");
         cudaMemcpy(input_char+fft_length*per_batch,input_char_cpu+fft_length*per_batch,fft_length*remain_batch*sizeof(char),cudaMemcpyHostToDevice);
         //read_char_array(input_char+fft_length*per_batch*2,simulate_input_char,fft_length*remain_batch*2);
         //先读取余下的数据
         char2float(input_char+fft_length*per_batch,input_half+input_half_offset,fft_length,remain_batch,thread_num);
         #ifdef PRINT_MEMORY
         print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
         #endif
         ...
         if(per_batch-remain_batch<window_size-step)
         {
             ...
             printf("first step\n");
             char2float_reflect(input_char+(per_batch+remain_batch-step)*fft_length-1,input_half+(fft_length+2)*(window_size+remain_batch),fft_length,per_batch-remain_batch,thread_num);
             #ifdef PRINT_MEMORY
             print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
             #endif
             ...
             printf("second step\n");
             char2float_reflect(input_char+(2*remain_batch-step)*fft_length-1,input_half+input_half_offset,fft_length,(window_size-step)-(per_batch-remain_batch),thread_num);
             #ifdef PRINT_MEMORY
             print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
             #endif
             ...
             complex_modulus_squared(input_half+input_half_offset,input_float+input_float_offset,fft_length/2,((window_size-step)-(per_batch-remain_batch)),thread_num);
             #ifdef PRINT_MEMORY
             print_data_float(input_float,0,(fft_length/2+1)*batch_buffer_size,fft_length/2+1);
             #endif
        ...
        //缓冲区空间够用,可以一次性完成反射
         else
         {
             ...
             char2float_reflect(input_char+(per_batch+remain_batch-step)*fft_length*2-1,input_half+(fft_length+2)*(window_size+remain_batch),fft_length,window_size-step,thread_num);
             #ifdef PRINT_MEMORY
             print_data_half(input_half,0,(fft_length+2)*batch_buffer_size,fft_length+2);
             #endif
             ...
             //计算量为remain_batch+window_size-step
             complex_modulus_squared(input_half+input_half_offset,input_float+input_float_offset,fft_length/2,(remain_batch+window_size-step),thread_num);
             #ifdef PRINT_MEMORY
             print_data_float(input_float,0,(fft_length/2+1)*batch_buffer_size,fft_length/2+1);
             #endif
